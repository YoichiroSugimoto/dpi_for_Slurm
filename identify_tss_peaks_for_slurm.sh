...
case ${decomposition} in
  "y"|"Y")
    ...
    rake ${RAKE_OPT} prep
    rake ${RAKE_OPT} bw
    rake ${RAKE_OPT} pool_ctss
    rake ${RAKE_OPT} tc
    rake ${RAKE_OPT} tc_short
    rake ${RAKE_OPT} tc_long
    rake ${RAKE_OPT} decompose_smoothing
    rake ${RAKE_OPT} comp_ctss
    rake ${RAKE_OPT} final
    rake ${RAKE_OPT} threshold
    )
  ;;
  "n"|"N")
    (
    ...
    do 
      srun --export=all--export=RAKE_TARGET=$X  --export="${qsub_v}" ./scripts/qsub.sh
    done

    ### 02 - 05 ###
    srun --export=all --export=${qsub_v} --export=RAKE_TARGET=pool_ctss ./scripts/qsub.sh
    srun --export=all --export=${qsub_v} --export=RAKE_TARGET=tc ./scripts/qsub.sh
    srun --export=all --export=${qsub_v} --export=RAKE_TARGET=tc_short ./scripts/qsub.sh
    srun --export=all --export=${qsub_v} --export=RAKE_TARGET=tc_long ./scripts/qsub.sh

    ### 06 ###
    for X in $(rake --dry-run decompose_smoothing 2>&1 | grep Execute | cut -f 5 -d ' ' | grep  '.bed$' )
    do
      srun --export=all --export=${qsub_v} --export=RAKE_TARGET=$X ./scripts/qsub.sh
    done

    ### 07 - 09 ###
    srun --export=all --export=${qsub_v} --export=RAKE_TARGET=comp_ctss ./scripts/qsub.sh
    srun --export=all --export=${qsub_v} --export=RAKE_TARGET=final ./scripts/qsub.sh
    srun --export=all --export=${qsub_v} --export=RAKE_TARGET=threshold ./scripts/qsub.sh
    )
  ;;
  *)
    usage;;
