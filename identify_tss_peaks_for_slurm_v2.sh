...
case ${decomposition} in
  ...
  "y"|"Y")
    ...
    do
	sbatch --export=ALL,RAKE_TARGET=$X,${qsub_v} --job-name=01_to_05 ./scripts/sbatch.sh
    done

    ### 02 - 05 ###
    sbatch --export=ALL,RAKE_TARGET=pool_ctss,${qsub_v} --job-name=01_to_05 --dependency=singleton ./scripts/sbatch.sh
    sbatch --export=ALL,RAKE_TARGET=tc,${qsub_v} --job-name=01_to_05 --dependency=singleton ./scripts/sbatch.sh
    sbatch --export=ALL,RAKE_TARGET=tc_short,${qsub_v} --job-name=01_to_05 --dependency=singleton ./scripts/sbatch.sh
    sbatch --wait --export=ALL,RAKE_TARGET=tc_long,${qsub_v} --job-name=01_to_05 --dependency=singleton ./scripts/sbatch.sh

    ### 06 ###
    for X in $(rake --dry-run decompose_smoothing 2>&1 | grep Execute | cut -f 5 -d ' ' | grep  '.bed$' )
    do
      sbatch --export=ALL,RAKE_TARGET=$X,${qsub_v} --job-name=04_and_06_to_09 ./scripts/sbatch.sh
    done

    ### 07 - 09 ###
    sbatch --export=ALL,RAKE_TARGET=comp_ctss,${qsub_v} --job-name=04_and_06_to_09 --dependency=singleton ./scripts/sbatch.sh
    sbatch --export=ALL,RAKE_TARGET=final,${qsub_v} --job-name=04_and_06_to_09 --dependency=singleton ./scripts/sbatch.sh
    sbatch --wait --export=ALL,RAKE_TARGET=threshold,${qsub_v} --job-name=04_and_06_to_09 --dependency=singleton ./scripts/sbatch.sh
    )
  ;;
  *)
    usage;;
